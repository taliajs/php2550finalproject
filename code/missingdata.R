...
miss_var_summary(isolates) %>% filter(pct_miss >50) %>% kbl(caption = "Missing Values for each Variable") %>% kable_classic()
miss_var_table(isolates) %>% kbl(caption = "Missing Values for each Variable") %>% kable_classic()
